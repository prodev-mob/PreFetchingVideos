...
//
protocol WatchCountIncreseDelegate {
    func didfinishWatchCount()
}
...
class ASVideoPlayerController: NSObject, NSCacheDelegate {
    ...
    //
    var delegate:WatchCountIncreseDelegate?
    ...
    func playVideo(withLayer layer: AVPlayerLayer, url: String) {
        ...
        DispatchQueue.main.async {
            if let videoContainer = self.videoCache.object(forKey: url as NSString),
                ...
                if let duration = videoContainer.player.currentItem?.asset.duration {
                    let seconds = CMTimeGetSeconds(duration)
                    print("Seconds :: \(seconds)")
                    
                    DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 1.0) {
                        // check if player is still playing
                        if videoContainer.player.rate != 0 {
                            //print("OK")
    ...
    @objc func playerDidFinishPlaying(note: NSNotification) {
        ...
        if let currentItem = currentPlayer.currentItem, currentItem == playerItem {
            ...
            /*if let duration = currentPlayer.currentItem?.asset.duration {
                let seconds = CMTimeGetSeconds(duration)
                print("Seconds :: \(seconds)")
